...
echo -e "${r} [ ${g}04 ${r}] ${c}Ransower Generator Script Termux "
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "Sal.py" ]; then
        ...
        memek "${r}[ ${g}✓ ${r}]${c} Sedang menjalankan tools..!!"
        echo ""
        python3 Sal.py || kontol "Terjadi kesalahan."
    else
        echo ""
        memek "${r}[ ${g}+ ${r}]${y} File tidak ada, mengclone ulang..!!${c}"
        echo ""
        ...
        git clone --depth=32 https://github.com/QrwszXnXnchommed0e56/Loar || kontol "Gagal cloning repository"
        cd "$ANK_IDIOT" || kontol "Gagal masuk ke direktori github."
        python3 Sal.py || kontol "Terjadi kesalahan saat menjalankan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repository github..!!${c}"
    echo ""
...
y='\033[33;1m'
b='\033[34;1m'
DIR_PATH="/sdcard/CEK_IMEI_RESULT"
mkdir -p "$DIR_PATH"
bar() {
f=(
     "[\e[1;91m■\e[0m□□□□□□□□□]"
...
while true; do
     for fra in "${f[@]}"; do
     printf "\r$fra \033[41;1m\033[1;32m🔍 Sedang Cek IMEI \033[0m "
          sleep 0.12
     done
done
}
get_json_value() {
echo "$1" | jq -r "$2 // \"-\"" 2>/dev/null | sed 's/^null$/-/'
}
cek_imei() {
    local imei="$1"
    python3 << EOF
...
import sys
imei_input = '$imei'
def check_imei(imei: str, service_id: str = '1234'):
    url = 'https://free.imeicheck.com/free-check.php'    
    headers = {
        'User-Agent': 'Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36',
        'Accept': 'application/json, text/javascript, */*; q=0.01',
        'Accept-Language': 'id,en-US;q=0.9,en;q=0.8',
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'Origin': 'https://imeicheck.com',
        'Referer': 'https://imeicheck.com/',
        'Connection': 'keep-alive'
    }    
    visitor_uuid = str(uuid.uuid4())
    random_token = secrets.token_hex(16)
    req_signature = hashlib.md5(random_token.encode()).hexdigest()    
    try:
        cf_response = requests.get('https://cf-solver-three.vercel.app/api/imeitoken', timeout=15)
        cf_token = cf_response.json()['token']
    except:
        cf_token = ''    
    data = {
    ...
    try:
        ...
        html_content = resp_json.get('response', '')        
        brand = re.search(r'Brand:\s*(.*?)<br>', html_content)
        model = re.search(r'Model:\s*(.*?)<br>', html_content)
        model_name = re.search(r'Model Name:\s*(.*?)<br>', html_content)
        hasil_imei = {
            'IMEI': imei,
        ...
        try:
            ai_response = requests.post('https://cf-solver-three.vercel.app/api/aidevice', json=hasil_imei, timeout=30)
            if ai_response.status_code == 200:
                ai_data = ai_response.json()
                hasil_imei['Details'] = ai_data
            else:
                hasil_imei['Details'] = {'error': 'Tidak bisa mengambil data lengkap'}
        except Exception as e:
            hasil_imei['Details'] = {'error': f'Gagal koneksi ke AI: {str(e)}'}        
        return hasil_imei
    except Exception as e:
        return {'ERROR': str(e), 'IMEI': imei}
hasil = check_imei(imei_input)
...
local data_json="$1"
echo -e "${r}======================================"
echo -e "${g}   📱 INFORMASI LENGKAP DEVICE 📱 "
echo -e "${r}======================================"
echo ""
imei=$(get_json_value "$data_json" '.IMEI')
brand=$(get_json_value "$data_json" '.Brand')
model=$(get_json_value "$data_json" '.Model')
model_name=$(get_json_value "$data_json" '.Model_Name')
echo -e "${b}================================================"
    echo -e "${y}📱 IMEI  : ${p}$imei"
    echo -e "${y}🏷️  Brand  : ${p}$brand"
    echo -e "${y}📟 Model  : ${p}$model"
    echo -e "${y}✨ Model Name  : ${p}$model_name"
echo -e "${b}================================================"
details=$(echo "$data_json" | jq -c '.Details // {}' 2>/dev/null)
if [ "$details" != "{}" ] && [ "$details" != "null" ] && [ -n "$details" ]; then
os=$(get_json_value "$details" '.os')      
chipset_name=$(get_json_value "$details" '.chipset.name // .chipset')
cpu=$(get_json_value "$details" '.chipset.cpu')
gpu=$(get_json_value "$details" '.chipset.gpu')
process=$(get_json_value "$details" '.chipset.process')
echo ""
echo -e "${g}⚙️  SISTEM & CHIPSET"
echo -e "${b}================================================"
echo -e "${y}📱 OS  : ${p}$os"
echo -e "${y}🧠 Chipset  : ${p}$chipset_name"
[ "$cpu" != "-" ] && echo -e "${y}⚡ CPU  : ${p}$cpu"
[ "$gpu" != "-" ] && echo -e "${y}🎮 GPU  : ${p}$gpu"
[ "$process" != "-" ] && echo -e "${y}🔧 Proses  : ${p}$process"
echo -e "${b}================================================"       
display_type=$(get_json_value "$details" '.display.type')
display_size=$(get_json_value "$details" '.display.size')
display_res=$(get_json_value "$details" '.display.resolution')
refresh_rate=$(get_json_value "$details" '.display.refresh_rate')
brightness=$(get_json_value "$details" '.display.brightness')
protection=$(get_json_value "$details" '.display.protection')
echo ""       
echo -e "${g}🖥️  DISPLAY"
echo -e "${b}================================================"
[ "$display_type" != "-" ] && echo -e "${y}📺 Tipe  : ${p}$display_type"
[ "$display_size" != "-" ] && echo -e "${y}📏 Ukuran  : ${p}$display_size"
[ "$display_res" != "-" ] && echo -e "${y}🔍 Resolusi  : ${p}$display_res"
[ "$refresh_rate" != "-" ] && echo -e "${y}🔄 Refresh Rate  : ${p}$refresh_rate"
[ "$brightness" != "-" ] && echo -e "${y}☀️ Brightness : ${p}$brightness"
[ "$protection" != "-" ] && echo -e "${y}🛡️ Protection  : ${p}$protection"
echo -e "${b}================================================"
battery_cap=$(get_json_value "$details" '.battery.capacity')
battery_charge=$(get_json_value "$details" '.battery.charging')   
echo ""   
echo -e "${g}🔋 BATTERY"
echo -e "${b}================================================"
[ "$battery_cap" != "-" ] && echo -e "${y}⚡ Kapasitas  : ${p}$battery_cap"
[ "$battery_charge" != "-" ] && echo -e "${y}🔌 Pengisian  : ${p}$battery_charge"
echo -e "${b}================================================"
rear_type=$(echo "$details" | jq -r '.camera.rear | type' 2>/dev/null)        
if [ "$rear_type" = "array" ]; then
rear_res=$(echo "$details" | jq -r '.camera.rear[0].resolution // "-"' 2>/dev/null)
rear_ap=$(echo "$details" | jq -r '.camera.rear[0].aperture // "-"' 2>/dev/null)
rear_res2=$(echo "$details" | jq -r '.camera.rear[1].resolution // ""' 2>/dev/null)
rear_ap2=$(echo "$details" | jq -r '.camera.rear[1].aperture // ""' 2>/dev/null)
elif [ "$rear_type" = "object" ]; then
rear_res=$(echo "$details" | jq -r '.camera.rear.resolution // "-"' 2>/dev/null)
rear_ap=$(echo "$details" | jq -r '.camera.rear.aperture // "-"' 2>/dev/null)
    rear_res2=""
else
    rear_res="-"
    rear_ap="-"
...
front_res=$(echo "$details" | jq -r '.camera.front.resolution // "-"' 2>/dev/null)
front_ap=$(echo "$details" | jq -r '.camera.front.aperture // "-"' 2>/dev/null)
video=$(echo "$details" | jq -r '.camera.video // "-"' 2>/dev/null)
echo ""     
echo -e "${g}📸 KAMERA"
echo -e "${b}================================================"
if [ "$rear_res" != "-" ] && [ "$rear_res" != "" ]; then
    if [ "$rear_ap" != "-" ] && [ "$rear_ap" != "" ]; then
       echo -e "${y}📷 Kamera Utama : ${p}$rear_res (f/$rear_ap)"
          else
      echo -e "${y}📷 Kamera Utama : ${p}$rear_res"
   fi
fi
if [ -n "$rear_res2" ] && [ "$rear_res2" != "" ] && [ "$rear_res2" != "-" ]; then
     if [ -n "$rear_ap2" ] && [ "$rear_ap2" != "" ]; then
          echo -e "${y}📷 Kamera kedua : ${p}$rear_res2 (f/$rear_ap2)"
             else
      echo -e "${y}📷 Kamera kedua : ${p}$rear_res2"
    fi
fi
if [ "$front_res" != "-" ] && [ "$front_res" != "" ]; then
   if [ "$front_ap" != "-" ] && [ "$front_ap" != "" ]; then
          echo -e "${y}🤳 Kamera Depan : ${p}$front_res (f/$front_ap)"
      else
           echo -e "${y}🤳 Kamera Depan : ${p}$front_res"
    fi
fi
[ "$video" != "-" ] && [ "$video" != "" ] && echo -e "${y}🎥 Video  : ${p}$video"
echo -e "${b}================================================"
ram=$(get_json_value "$details" '.memory.ram')
storage=$(get_json_value "$details" '.memory.storage')
card_slot=$(get_json_value "$details" '.memory.card_slot')      
echo ""
echo -e "${g}💾 MEMORI & STORAGE"
echo -e "${b}================================================"
[ "$ram" != "-" ] && echo -e "${y}🧠 RAM  : ${p}$ram "
[ "$storage" != "-" ] && echo -e "${y}💽 Storage  : ${p}$storage"
[ "$card_slot" != "-" ] && echo -e "${y}📇 Card Slot  : ${p}$card_slot"
echo -e "${b}================================================"
price_type=$(echo "$details" | jq -r '.price | type' 2>/dev/null)
if [ "$price_type" = "object" ]; then
  price_amt=$(get_json_value "$details" '.price.amount')
  price_curr=$(get_json_value "$details" '.price.currency')
  price_note=$(get_json_value "$details" '.price.note')
if [ "$price_amt" != "-" ] && [ "$price_curr" != "-" ]; then
     price_display="$price_curr $price_amt"
[ "$price_note" != "-" ] && [ "$price_note" != "" ] && price_display="$price_display ($price_note)"
else
   price_display="-"
    fi
else
   price_display=$(get_json_value "$details" '.price')
fi
release=$(get_json_value "$details" '.release_date')
 echo ""      
echo -e "${g}💰 HARGA & RILIS"
echo -e "${b}================================================"
[ "$price_display" != "-" ] && [ "$price_display" != "" ] && echo -e "${y}💵 Harga  : ${p}$price_display"
[ "$release" != "-" ] && echo -e "${y}📅 Rilis  : ${p}$release"
echo -e "${b}================================================"
image_url=$(get_json_value "$details" '.image')
if [ "$image_url" != "-" ] && [ "$image_url" != "null" ] && [ "$image_url" != "" ]; then
echo ""
echo -e "${g}🖼️  GAMBAR DEVICE"
echo -e "${b}================================================"
echo -e "${y}🔗 URL : ${c}$image_url"
echo -e "${b}================================================"
fi
  else
      echo -e "${r}⚠️ Detail spesifikasi tidak tersedia ⚠️"
fi
echo ""
}
main() {
...
echo ""
echo -e "${g}"
read -p "Masukkan IMEI target (15 digit) : " imei_number
echo ""  
if [ -z "$imei_number" ]; then
     clear 
     echo ""
     echo -e "${r}Jangan kosong dongs :v"
     echo ""
fi
if [[ ! "$imei_number" =~ ^[0-9]{15}$ ]]; then
     clear 
     echo ""
     echo -e "${r}IMEI harus 15 digit angka!"
     echo ""
fi    
bar &
anim_pid=$!    
hasil=$(cek_imei "$imei_number")   
kill $anim_pid 2>/dev/null
wait $anim_pid 2>/dev/null
printf "\r\033[K"   
echo ""
if echo "$hasil" | grep -q "ERROR"; then
     echo -e "${c}💡 IMEI : ${y}$imei_number"
     echo -e "${r}⚠️ Tidak ditemukan atau error jaringan."
else
     display_result "$hasil"        
     timestamp=$(date +"%Y%m%d_%H%M%S")
     save_file="$DIR_PATH/imei_${imei_number}_${timestamp}.json"
     echo "$hasil" | jq '.' > "$save_file" 2>/dev/null
     echo -e "${g}💾 Hasil disimpan di : ${y}$save_file${n}"
fi    
    echo ""
    echo -e "${c}✨ Thanks you for By.Thxyzz404 ✨"
    echo ""
}
main
echo ""
