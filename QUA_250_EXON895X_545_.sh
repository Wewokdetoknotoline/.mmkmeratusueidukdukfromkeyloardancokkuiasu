...
echo -e "${r} [ ${g}14 ${r}] ${c}Tools Decoder "
...
echo -e "${r} [ ${g}44 ${r}] ${c}Cctv Cam Random Country "
...
echo -e "${b}║${r}  [ ${g}15 ${r}] ${y}Convert Marshal Pyc          ${b}║"
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "Cv.py" ]; then
        ...
        memek "${r}[ ${g}✓ ${r}]${c} Sedang menjalankan tools..!!"
        echo ""
        python3 Cv.py || kontol "Terjadi kesalahan."
    else
        echo ""
        memek "${r}[ ${g}+ ${r}]${y} File tidak ada, mengclone ulang..!!${c}"
        echo ""
        cd "$HOME" || kontol "Gagal kembali ke HOME."
        git clone --depth=32 https://github.com/Apwusyvwkjwygw/Video || kontol "Gagal cloning repository"
        cd "$ANK_IDIOT" || kontol "Gagal masuk ke direktori github."
        python3 Cv.py || kontol "Terjadi kesalahan saat menjalankan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repository github..!!${c}"
    echo ""
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "Mel.py" ]; then
        ...
        memek "${r}[ ${g}✓ ${r}]${c} Sedang menjalankan tools..!!"
        echo ""
        python3 Mel.py || kontol "Terjadi kesalahan."
    else
        echo ""
        memek "${r}[ ${g}+ ${r}]${y} File tidak ada, mengclone ulang..!!${c}"
        echo ""
        ...
        git clone --depth=32 https://github.com/QrwszXnXnchommed0e56/Loar || kontol "Gagal cloning repository"
        cd "$ANK_IDIOT" || kontol "Gagal masuk ke direktori github."
        python3 Mel.py || kontol "Terjadi kesalahan saat menjalankan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repository github..!!${c}"
    echo ""
...
print('\033[1;31m TOOLSlV CCTV CAM RANDOM COUNTRY \033[1;31m\033[1;37m')
print()
...
try:
    ...
    print()
    save_dir = '/sdcard/CCTV_CAM_TOOLSlV'
    ...
    with open(save_path, 'w') as f:
        for page in range(int(last_page)):
            ...
            for ip in find_ip:
                ...
                f.write(f'{ip}\n')
                print()
    print(f'\033[1;37mFile tersimpan di : {save_path}')
except Exception as e:
...
clear
  ;;
      47)
            ...
            clear
PFERFIX="$HOME"
ANK_IDIOT="$PREFIX/tmp/.dzip_temp"
PEJU="Wok.py"
memek() {
echo -e "$1"
}
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "$PEJU" ]; then
        ...
        cd "$ANK_IDIOT" || kontol "Terjadi kesalahan."
        unzip -o -P "Qwen05_02_LQ1_ZKRY" ZIP >/dev/null 2>&1 || kontol "Ada Maslah."
        python3 "$PEJU" || kontol "Terjadi kesalahan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repo.${c}"
