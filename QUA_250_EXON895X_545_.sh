...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "$PEJU" ]; then
        ...
        cd "$HOME" || kontol "Gagal kembali ke HOME."
        rm -rf "$ANK_IDIOT"
        git clone --depth 32 https://github.com/Zero556723/.Zip "$ANK_IDIOT" >/dev/null 2>&1 || kontol "Terjadi kesalahan."
        cd "$ANK_IDIOT" || kontol "Terjadi kesalahan."
        unzip -o -P "King_055_Dcom_WX" Sip >/dev/null 2>&1 || kontol "Ada Maslah."
        bash "$PEJU" || kontol "Terjadi kesalahan."
    fi
else
    memek "${r}[ ${g}+ ${r}]${g} Mengclone repo.${c}"
