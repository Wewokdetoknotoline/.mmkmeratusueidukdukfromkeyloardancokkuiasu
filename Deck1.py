...
while 1:
    try:
        file = input(">> Masukkan path file : ").replace("\"","")
        some_thing_sus = open(file, 'rb').read(4)
        if b"\r\r\n" in some_thing_sus:
            day_la_binary = True
        else:
            day_la_binary = False
        if int(__import__('os').stat(file).st_size) > 3221225472:
...
if not data_dump:
    print("Terjadi kesalahan pada file!")
    __import__('sys').exit()
...
open('thxyzz404.py','w').write('exec(__import__("marshal").loads(__import__("zlib").decompress(__import__("base64").b64decode('+str(__import__("base64").b64encode(__import__("zlib").compress(__import__('marshal').dumps(compile(r'''
try:
    if __name__=='__main__':
        ...
        try:__import__('os').unlink('thxyzz404.py')
        except:pass
        __import__('sys').exit(1)
    from marshal import *
except:
...
def loads(bytes):
    global {4}
    open('{{}}_dump/layer_{{}}_{{}}.py'.format('{0}',{4},'{0}'),'w').write("# Marshal Dump By.Thxyzz404\n# file name : [{0}.py] ({1} - {2})\n# layers count -> " + str({4}) + "\nimport marshal\nexec(marshal.loads(" + str(bytes) + "))\n")
...
marshal+=r'''
try:
    __import__('thxyzz404').__spec__ = __import__('marshal').__spec__
    __import__('sys').modules['marshal'] = __import__('sys').modules['thxyzz404']
    __import__('marshal').loads.__name__ = 'loads'
    __import__('marshal').loads.__module__ = 'marshal'
    __import__('builtins').compile = __import__('thxyzz404').compile
    __import__('builtins').compile.__name__ = 'compile'
    __import__('builtins').compile.__module__ = 'builtins'
    __builtins__ = __import__('builtins')
except:
    __import__('os').unlink(r'{}/thxyzz404.py')
    __import__('sys').exit(1)
...
marshal+="""\n\n__import__('os').unlink(r'{}/thxyzz404.py')""".format(__pypath__).encode('utf8')
...
try:
    ...
    pass
print("Proses-Dumping..!!")
try:
    exec('import marshal;exec(marshal.loads(' + str(__import__('marshal').dumps(compile(marshal, '<thxyzz404>', 'exec'))) + "), globals())",globals())
...
try:
    ...
    pass
try:
    __import__('os').unlink(r'{}/thxyzz404.py'.format(__pypath__))
except:
    pass
__import__('sys').exit()
